...
export const useContentManager = () => {
  ...
  const loadContent = () => {
    ...
    if (storedProjects) {
      ...
      const defaultProjects = getDefaultPortfolioProjects();
      setPortfolioProjects(defaultProjects);
      localStorage.setItem('portfolioProjects', JSON.stringify(defaultProjects));
    }

    // Load service pages
    const storedServices = localStorage.getItem('servicePages');
    if (storedServices) {
      setServicePages(JSON.parse(storedServices));
    } else {
      const defaultServices = getDefaultServicePages();
      setServicePages(defaultServices);
      localStorage.setItem('servicePages', JSON.stringify(defaultServices));
    }

    // Load homepage
    const storedHomePage = localStorage.getItem('homePage');
    if (storedHomePage) {
      setHomePage(JSON.parse(storedHomePage));
    } else {
      const defaultHomePage = getDefaultHomePage();
      setHomePage(defaultHomePage);
      localStorage.setItem('homePage', JSON.stringify(defaultHomePage));
    }
  };

  const savePortfolioProjects = (projects: PortfolioProject[]) => {
    setPortfolioProjects(projects);
    localStorage.setItem('portfolioProjects', JSON.stringify(projects));
  };

  const saveServicePages = (services: ServicePage[]) => {
    setServicePages(services);
    localStorage.setItem('servicePages', JSON.stringify(services));
  };

  const saveHomePage = (homepage: HomePage) => {
  ...
  const addPortfolioProject = (project: Omit<PortfolioProject, 'id'>) => {
    ...
    const updatedProjects = [...portfolioProjects, newProject];
    savePortfolioProjects(updatedProjects);
  };

  const updatePortfolioProject = (id: number, updates: Partial<PortfolioProject>) => {
  ...
  const deletePortfolioProject = (id: number) => {
    const updatedProjects = portfolioProjects.filter(project => project.id !== id);
    savePortfolioProjects(updatedProjects);
  };

  const addServicePage = (service: Omit<ServicePage, 'id'>) => {
    const newService = {
      ...service,
      id: Date.now()
    };
    const updatedServices = [...servicePages, newService];
    saveServicePages(updatedServices);
  };

  const updateServicePage = (id: number, updates: Partial<ServicePage>) => {
    const updatedServices = servicePages.map(service =>
      service.id === id ? { ...service, ...updates } : service
    );
    saveServicePages(updatedServices);
  };

  const deleteServicePage = (id: number) => {
    const updatedServices = servicePages.filter(service => service.id !== id);
    saveServicePages(updatedServices);
  };
