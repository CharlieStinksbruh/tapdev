...
import { useContentManager, PortfolioProject } from '../../hooks/useContentManager';
import { 
  Plus, 
  Edit, 
  Trash2, 
...
const PortfolioManager: React.FC = () => {
  const { portfolioProjects, addPortfolioProject, updatePortfolioProject, deletePortfolioProject } = useContentManager();
  ...
  const handleSave = () => {
    if (isCreating) {
      addPortfolioProject(formData as Omit<PortfolioProject, 'id'>);
      setIsCreating(false);
    } else if (editingProject) {
      updatePortfolioProject(editingProject.id, formData);
      setIsEditing(false);
  ...
  const handleToggleEnabled = (id: number, enabled: boolean) => {
    updatePortfolioProject(id, { enabled });
  };
