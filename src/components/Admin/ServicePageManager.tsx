...
import { useContentManager, ServicePage } from '../../hooks/useContentManager';
import { 
  Plus, 
  Edit, 
  Trash2, 
...
const ServicePageManager: React.FC = () => {
  const { servicePages, addServicePage, updateServicePage, deleteServicePage } = useContentManager();
  ...
  const handleDuplicate = (service: ServicePage) => {
    ...
    addServicePage(duplicatedService);
  };

  const handleSave = () => {
    if (isCreating) {
      addServicePage(formData as Omit<ServicePage, 'id'>);
      setIsCreating(false);
    } else if (editingService) {
      updateServicePage(editingService.id, formData);
      setIsEditing(false);
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="text-gray-300">#{service.order}</span>
              </div>
              <div className="flex items-center justify-between text-sm">
